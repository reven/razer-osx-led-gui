...
/**
 * Write device file "set_gm_toggle"
 *
 * Sets the logo lighting state to the ASCII number written to this file.
 */
int razer_attr_write_set_gm_toggle(IOUSBDeviceInterface **usb_dev, const char *buf, int count) {
    unsigned char state = (unsigned char)strtol(buf, NULL, 10);
    struct razer_report report = razer_chroma_misc_gm_toggle(state);
    char buffer[90];
