 ...
 */
/**
 * Toggled wether gaming mode is on or off
 *
 * If 0 should mean that the gaming mode is off
 * If 1 should mean that the gaming mode is on
 */
struct razer_report razer_chroma_misc_gm_toggle(unsigned char state) {
    struct razer_report report = get_razer_report(0x02, 0x06, 0x02);
    report.arguments[0] = 0x00; // ?? Variable storage maybe
    report.arguments[1] = clamp_u8(state, 0x00, 0x01); // State
